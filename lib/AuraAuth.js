 ...
 *
 * Node.js port of tebrown/cummins_hacs `aura_auth.py`. Replicates the SSO
 * dance without a browser: Cognito hosted UI -> SAML federation into Cummins'
 ...
 *
 * KNOWN RISK — TLS fingerprinting: Salesforce's edge (server: sfdcedge)
 * fingerprints the TLS ClientHello (JA3/JA4). Python `requests` gets served a
 * generic template missing the Aura bootstrap; a Chrome fingerprint gets the
 * real page. Node's TLS stack is also OpenSSL-based, so this login MAY be
 * blocked the same way depending on Salesforce's current rules. If it is,
 * login fails at the "Aura bootstrap not found" step — callers should map
 * that error to advice to use the refresh-token pairing method instead
 * (docs/PLAN.md). Cognito token refresh and the telemetry API are NOT behind
 * this edge and always work.
 *
