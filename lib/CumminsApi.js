...
class CumminsApi {
  ...
  async rawRequest(method, path, options = {}) {
    ...
    try {
      const res = await httpRequest(url, { method, headers, body });
      // Generous slice: the server's 422 replies carry the full allowed-value
      // enum, which is exactly the payload worth reading in full.
      return { status: res.status, body: (res.body || '').slice(0, 2000) };
    } catch (err) {
  ...
  /**
   * Send a command to a generator.
   *
   * Endpoint and body shape were established empirically (see
   * docs/HA-INTEGRATIONS.md "Cloud command endpoint"): the mobile API takes
   * `POST /Assets/SendCommand?id=<assetId>` with a body matching the objects
   * `/Assets/Commands` returns. It rejected a bogus CommandString with a 422
   * listing every allowed value, which is how COMMANDS below was obtained —
   * no command had to be executed to learn the protocol.
   *
   * @param {string} assetId
   * @param {string} commandName one of COMMANDS
   * @param {Array<any>} [properties] command arguments; empty for the simple
   *   start/stop/exercise commands. Parameterised commands (SetStandby,
   *   SetExerciseSchedule) need a shape that is still unknown.
   */
  async sendCommand(assetId, commandName, properties = []) {
    return this._request('POST', '/Assets/SendCommand', {
      params: { id: assetId },
      json: {
   ...
   *
   * The endpoint is solved (POST /Assets/SendCommand?id=, see sendCommand).
   * This is retained as a diagnostic for generators whose commands misbehave,
   * and as a regression check that the path still exists.
   *
  ...
  async probeCommandEndpoints(assetId) {
    ...
    // Round 1 established that /Assets/SendCommand is the endpoint (it
    // answered 400 "Required parameter id is missing!" while every other
    // path 404'd). These narrow down the body shape it wants, reusing the
    // exact field names /Assets/Commands hands back.
    const commandObject = { DestinationId: assetId, CommandString: BOGUS, Properties: [] };

    /** @type {Array<{label: string, method: string, path: string, options?: any}>} */
    const candidates = [
      { label: 'SendCommand?id=  {DestinationId, CommandString, Properties}', method: 'POST', path: '/Assets/SendCommand', options: { params: { id: assetId }, json: commandObject } },
      { label: 'SendCommand?id=  [{DestinationId, CommandString, Properties}]', method: 'POST', path: '/Assets/SendCommand', options: { params: { id: assetId }, json: [commandObject] } },
      { label: 'SendCommand?id=  {CommandString}', method: 'POST', path: '/Assets/SendCommand', options: { params: { id: assetId }, json: { CommandString: BOGUS } } },
      { label: 'SendCommand?id=  {Commands:[...]}', method: 'POST', path: '/Assets/SendCommand', options: { params: { id: assetId }, json: { Commands: [commandObject] } } },
      // Kept as controls so a regression in the finding is obvious
      { label: '(control) POST /Assets/{id}/command/{name}', method: 'POST', path: `/Assets/${id}/command/${BOGUS}`, options: { json: {} } },
    ];
